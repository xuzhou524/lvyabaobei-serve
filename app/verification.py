...
def _latest_code(db: Session, email: str) -> EmailVerificationCode | None:
    return (
...
def assert_can_send_code(db: Session, email: str) -> None:
    normalized = normalize_email(email)
    latest = _latest_code(db, normalized)
    if not latest:
        return
...
def save_verification_code(db: Session, email: str, code: str) -> int:
    normalized = normalize_email(email)
    now_ms = beijing_now_ms()
    expires_at = now_ms + settings.verification_code_expire_minutes * 60 * 1000

    db.query(EmailVerificationCode).filter(EmailVerificationCode.email == normalized).delete()
...
def verify_code(db: Session, email: str, code: str) -> None:
    normalized = normalize_email(email)
    record = _latest_code(db, normalized)
    if not record:
